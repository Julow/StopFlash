...
StopFlashUI.prototype.setElements = function(elements)
{
	...
	this.mainTab.clear()
		.append(this.whitelist
			...
			.set('title', this.popup.isWhitelist? chrome.i18n.getMessage('moins_whitelist') : chrome.i18n.getMessage('add_whitelist')))
	...
	if(elements != null && elements.length > 0)
	{
		...
		elementsCount.text(chrome.i18n.getMessage((elements.length > 1)? 'finds' : 'find', [elements.length]));
	...
	this.whitelistTab.clear()
		.append(new Builder('p')
			.text(chrome.i18n.getMessage((this.popup.whitelist.length > 1)? 'whitelists' : 'whitelist', [this.popup.whitelist.length])));
...
function StopFlashElement(popup, data)
{
	...
	this.className('flash-element')
		...
		.append(new Builder('div')
			.className('flash-menu')
			.append(new Builder('a')
				.text(chrome.i18n.getMessage(this.data.blocked? 'unlock' : 'lock'))
				.event('click', function()
				{
...
StopFlashTabs.prototype.setTab = function(index)
{
	...
	{
		if(index === i)
			this.tabs[i].css('position', 'relative').css('opacity', '1');
		else
			this.tabs[i].css('position', '').css('opacity', '0.3');
	}
